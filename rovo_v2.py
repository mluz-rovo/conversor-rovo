...
# FORÇAR REINÍCIO - VERSÃO VERDE FINAL
st.set_page_config(page_title="ROVO VERDE", layout="wide")

# Pintar o site de verde para confirmar a atualização
st.markdown("<style>.stApp { background-color: #d4edda; }</style>", unsafe_allow_html=True)

st.sidebar.title("🌿 MENU ROVO")
st.sidebar.success("✅ CÓDIGO ATUALIZADO")
                    ...
                    for table in tab:
                        ...
                        for r_idx, r in enumerate(table):
                            ...
                            if any(t in r_str for t in tams_ref):
                                h = [str(x).replace('\n', ' ').strip() for x in r]; s_idx = r_idx + 1; break
                        if s_idx == -1: continue
                        ...
                        for i in range(s_idx, len(table)):
                            ...
                            if "€" in r_full:
                                ...
                                for c in row:
                                    if "€" in str(c):
                                        p_v = pd.to_numeric(str(c).replace('€','').replace(',','.').replace(' ',''), errors='coerce'); break
                                    ...
                                    if t_ok:
                                        ...
                                        if qtd and qtd > 0:
                                            lista_dados.append({
                                                ...
                                                'Cor': cor, 'Tamanho': t_ok, 'TOTAL': qtd*p_v, 'Destino': dest, 'CPO': ""
                                            })
