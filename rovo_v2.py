...
# Versão 2.2 - Ajuste de Colunas Stussy (A-P) e Nicholson Estável
...
arquivo = st.file_uploader("Submeter ficheiro", type=formatos)
...
if arquivo:
    try:
        ...
        if arquivo.name.endswith('.xlsx'):
            ...
            if cliente == "Stussy":
                ...
                for i, row in df.iloc[1:].iterrows():
                    # Se tem 16 colunas (A-P), o preço (Pr.Unit.Moeda) costuma estar na P (15) ou O (14)
                    # Vou usar a última coluna disponível para o preço Moeda
                    idx_preco = len(row) - 1 
                    idx_qtd = 12 if len(row) > 12 else idx_preco - 1
                    
                    q = pd.to_numeric(row[idx_qtd], errors='coerce')
                    p = pd.to_numeric(row[idx_preco], errors='coerce')
        ...
        # --- LÓGICA PDF STUDIO NICHOLSON (A QUE FUNCIONA) ---
